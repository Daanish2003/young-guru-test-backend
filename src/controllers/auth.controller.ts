...
export const loginHandler = asyncHandler(async (req, res): Promise<void> => {
  ...
  const {
    data: existingUser,
    error: getExistingUserError
  } = await tryCatch(getExistingUser(phoneNumber));
  ...
  if (!user) {
    ...
    } = await tryCatch(createNewUser(phoneNumber));
  ...
  const {
    data: jwtToken,
    error: generateJWTTokenError
  } = await tryCatch(generateJWT(user));
