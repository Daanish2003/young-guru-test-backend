...
function isSessionPayload(payload: JWTPayload): payload is SessionPayload {
  return typeof payload.id === "string" && typeof payload.phone === "string" && typeof payload.level === "string";
}
...
export const saveLoginInfo = async ({ userId, ipAddress }: { userId: string; ipAddress: string }) => {
